...
def _parse_version_ranges(range_str):
    """Parse '100-200,250-300' or '100-200 & 250-300' into [(100,200),(250,300)]."""
    ...
    for part in normalized.split(','):
        ...
        if '-' in part:
            lo, hi = part.split('-', 1)
            ranges.append((int(lo.strip().lstrip('vV')), int(hi.strip().lstrip('vV'))))
        else:
            v = int(part.lstrip('vV'))
            ranges.append((v, v))
    if not ranges:
        raise ValueError(f"Could not parse version ranges from: '{range_str}'")
    return ranges


def _version_range_label(ranges):
    """Convert [(100,200),(250,300)] to folder-safe string 'v100-v200_v250-v300'."""
    parts = []
    for lo, hi in ranges:
        parts.append(f'v{lo:03d}' if lo == hi else f'v{lo:03d}-v{hi:03d}')
    return '_'.join(parts)
...
class ComprehensiveHyperoptAnalyzer:
    ...
    def __init__(self, mode='pd', log_path=None, output_dir=None, target_metric='mean_val_accuracy',
                 min_experiments=3, verbose=False, versions=None):
        # Validate mode
        ...
        self.version_ranges = _parse_version_ranges(versions) if versions else []
        self.version_label = _version_range_label(self.version_ranges) if self.version_ranges else None
        if self.version_label:
            self.output_dir = self.output_dir / self.version_label
    ...
    def load_and_validate_data(self):
        ...
        # Apply version range filter before any other validation
        if self.version_ranges:
            def _in_ranges(ver_str):
                n = config.parse_version(ver_str)
                return n is not None and any(lo <= n <= hi for lo, hi in self.version_ranges)
            self.df = self.df[self.df['version'].apply(_in_ranges)].copy()
            if self.df.empty:
                raise ValueError(
                    f"No experiments found for version ranges: "
                    f"{[f'{lo}-{hi}' for lo, hi in self.version_ranges]}"
                )
            if self.verbose:
                print(f"Version filter applied ({self.version_label}): {len(self.df)} experiments")
    ...
    def _safe_savefig(self, fig, path, dpi=300, **kwargs):
        """Save figure, scaling it down if either pixel dimension exceeds renderer limit."""
        MAX_PIXELS = 60000  # safe margin below 2^16 = 65536
        w_in, h_in = fig.get_size_inches()
        w_px, h_px = w_in * dpi, h_in * dpi
        if w_px > MAX_PIXELS or h_px > MAX_PIXELS:
            scale = min(MAX_PIXELS / w_px, MAX_PIXELS / h_px)
            new_w, new_h = w_in * scale, h_in * scale
            print(f"  ⚠  Figure too large ({w_px:.0f}×{h_px:.0f} px); "
                  f"scaling to ({new_w*dpi:.0f}×{new_h*dpi:.0f} px)")
    ...
    def preprocess_parameters(self):
        ...
        for param in self.parameter_names:
            ...
            if param == 'dense_dropout':
                # Stored in CWT logs as a list string e.g. "[0.3]" or "[0.3, 0.5]"; extract mean
                def _parse_dense_dropout(v):
                    try:
                        return float(v)
                        ...
                        try:
                            ...
                            pass
                    return np.nan
                X[param] = values.apply(_parse_dense_dropout)

            elif param in ['learning_rate', 'l2_reg', 'l2_regularization', 'augment_fraction',
                         'conv_dropout', 'noise_std', 'brightness_range', 'contrast_range',
    ...
    def _compute_theoretical_best_config(self):
        """
        For each parameter with OFAT data, find the value that achieves the highest
        mean performance across all sweeps. Returns a list of dicts sorted by
        the magnitude of improvement (best_mean - overall_mean), descending.
        """
        _bool_params = {'batch_norm', 'use_batch_norm', 'class_weights', 'use_class_weights'}
        _int_params = {'batch_size', 'epochs', 'k_folds', 'early_stopping_patience',
                       'lr_reduction_patience', 'img_width', 'img_height', 'img_channels'}

        overall_mean = np.mean(self.y)
        entries = []
        ...
        for param, groups in self.ofat_results.items():
            ...
            # Aggregate (preprocessed_value → [performances]) across all sweeps
            value_perf = defaultdict(list)
            for group in groups:
                for val, perf in zip(group['target_values'], group['performance']):
                    value_perf[round(float(val), 10)].append(perf)

            if not value_perf:
                continue

            # Pick the value with the highest mean performance
            best_key = max(value_perf, key=lambda v: np.mean(value_perf[v]))
            best_mean = float(np.mean(value_perf[best_key]))
            n_obs = len(value_perf[best_key])

            # Format to a human-readable value
            if param in self.encoders:
                try:
                    display = self.encoders[param].inverse_transform([int(round(best_key))])[0]
                except Exception:
                    display = best_key
            elif param in _bool_params:
                display = bool(round(best_key))
            elif param in _int_params:
                display = int(round(best_key))
            else:
                display = best_key  # float

            entries.append({
                'param': param,
                'best_value': display,
                'best_mean': best_mean,
                'improvement': best_mean - overall_mean,
                'n_obs': n_obs,
            })

        # Sort by improvement magnitude so the most impactful params come first
        entries.sort(key=lambda e: e['improvement'], reverse=True)
        return entries
    ...
    def plot_ofat_analysis(self):
        ...
        plot_idx = 0
        sweep_colors = plt.cm.tab10.colors
        _bool_params = {'batch_norm', 'use_batch_norm', 'class_weights', 'use_class_weights'}
        ...
        for param, groups in self.ofat_results.items():
            ...
            ax = axes[plot_idx]
            is_bool = param in _bool_params

            total_points = 0
            agg = defaultdict(list)
            for sweep_idx, group in enumerate(groups):
                color = sweep_colors[sweep_idx % len(sweep_colors)]
                ax.scatter(group['target_values'], group['performance'],
                           alpha=0.6, s=40, color=color,
                           label=f'Sweep {sweep_idx + 1}')
                total_points += len(group['target_values'])
                for x, y in zip(group['target_values'], group['performance']):
                    agg[round(float(x), 10)].append(y)

            if total_points > 0:
                # Average line with error bars across sweeps
                if len(agg) >= 2:
                    xs = sorted(agg.keys())
                    means = [np.mean(agg[x]) for x in xs]
                    stds = [np.std(agg[x]) for x in xs]
                    ax.errorbar(xs, means, yerr=stds, color='black',
                                linewidth=1.5, marker='o', markersize=4,
                                capsize=3, zorder=5, label='Mean ± std')

                ax.set_xlabel(param.replace('_', ' ').title())
                ax.set_ylabel(self.target_metric.replace('_', ' ').title())
                ax.set_title(f'OFAT: {param} ({total_points} points)')
        ...
        self._safe_savefig(fig, plot_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_architecture_analysis(self):
        ...
        # Check if we have architecture parameters with actual variation
        has_conv = ('conv_filters' in self.df.columns and
                    self.df['conv_filters'].dropna().nunique() > 1)
        has_dense = ('dense_units' in self.df.columns and
                     self.df['dense_units'].dropna().nunique() > 1)
    ...
    def generate_comprehensive_report(self):
        ...
        if self.version_label:
            report_lines.append(f"**Version Filter:** {self.version_label}\n")
        ...
        top5 = (self.df[['version', self.target_metric]
                        + [c for c in ['std_val_accuracy', 'std_val_loss', 'total_training_time_minutes']
                           if c in self.df.columns]]
                .nlargest(5, self.target_metric)
                .reset_index(drop=True))
        metric_label = self.target_metric.replace('_', ' ').title()
        std_col = 'std_val_accuracy' if 'std_val_accuracy' in top5.columns else None
        for rank, row in top5.iterrows():
            version = row.get('version', 'N/A')
            score = row[self.target_metric]
            std_str = f" ± {row[std_col]:.4f}" if std_col else ""
            time_str = ""
            if 'total_training_time_minutes' in top5.columns and pd.notna(row.get('total_training_time_minutes')):
                time_str = f", {row['total_training_time_minutes']:.1f} min"
            report_lines.append(f"{rank + 1}. **{version}** — {metric_label}: {score:.4f}{std_str}{time_str}\n")
        report_lines.append("\n")

        # Theoretical best configuration from OFAT optima
        if self.ofat_results:
            report_lines.append("## Theoretical Best Configuration (OFAT Optima)\n\n")
            report_lines.append(
                "Each parameter is set to the value that achieved the highest mean performance "
                "across its OFAT sweeps, independently of the other parameters. "
                "Parameter interactions are not accounted for, so this represents an upper-bound "
                "estimate.\n\n"
            )
            theo_best = self._compute_theoretical_best_config()
            if theo_best:
                headers = ['Parameter', 'Optimal Value', 'Mean Perf. at Optimum',
                           'Delta vs. Overall Mean', 'N obs.']
                table_rows = []
                for e in theo_best:
                    val = e['best_value']
                    val_str = f"{val:.6g}" if isinstance(val, float) else str(val)
                    sign = "+" if e['improvement'] >= 0 else ""
                    table_rows.append([
                        e['param'],
                        val_str,
                        f"{e['best_mean']:.4f}",
                        f"{sign}{e['improvement']:.4f}",
                        str(e['n_obs']),
                    ])
                ...
                report_lines.append(_fmt_row(headers))
                report_lines.append(_fmt_sep())
                for row in table_rows:
                    report_lines.append(_fmt_row(row))
                report_lines.append("\n")
            else:
                report_lines.append("*Insufficient OFAT data to determine theoretical best configuration.*\n\n")
    ...
    def run_complete_analysis(self):
        ...
        if self.version_label:
            print(f"Version filter: {self.version_label}")
        ...
        plot_steps = [
            ('OFAT sensitivity', self.plot_ofat_analysis),
            ('architecture', self.plot_architecture_analysis),
            ('ANOVA', self.plot_anova_analysis),
            ('interaction', self.plot_interaction_analysis),
            ('pairwise surfaces', lambda: self.plot_pairwise_surfaces(top_n=4)),
            ('optimization timeline', self.plot_optimization_timeline),
        ]
        for step_name, fn in plot_steps:
            try:
                fn()
            except Exception as e:
                print(f"  ⚠  {step_name} plot failed: {e}")
...
def main():
    ...
    parser.add_argument('--versions', type=str, default=None,
                       help='Version number ranges to analyse, e.g. "100-200" or "100-200,250-300"')
