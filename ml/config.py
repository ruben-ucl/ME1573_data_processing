...
#
# Usage Examples:
#   - Multi-channel studies use ALL uncommented channels below
#   - Channel ablation mode tests all combinations of uncommented channels
#   - Single-channel mode: use --channel flag (e.g., --channel PD1_cmor2.5-0.5)
#   - Comment out any channels you want to exclude from multi-channel studies
#
# Example: To run PD1 vs PD2 comparison with cmor1.5-1.0 only:
#   Keep only: PD1_cmor1.5-1.0, PD2_cmor1.5-1.0
#   Comment out all others
#
# Example: To compare wavelets on PD1:
#   Keep only: PD1_cmor1.5-1.0, PD1_cmor2.5-0.5, PD1_mexh
#   Comment out all PD2 channels
#
CWT_DATA_DIR_DICT = {
    # ===== PD1 Channels =====
...
def get_model_dir(classifier_type='cwt_image', dataset_variant=None):
    """
    Get the model directory for a specific classifier and optional dataset variant.

    Args:
        classifier_type: 'cwt_image' or 'pd_signal'
        dataset_variant: Optional dataset variant name

    Returns:
        Path: Model directory path
    """
    if classifier_type == 'cwt_image':
        base_dir = CWT_OUTPUTS_DIR
    else:
        base_dir = PD_OUTPUTS_DIR
...
def load_config(config_path=None, classifier_type='pd_signal', **overrides):
    ...
    if config_path and Path(config_path).exists():
        try:
            with open(config_path, 'r') as f:
                ...
                deprecated_params = {'skip_time_ms'}
                file_config = {k: v for k, v in file_config.items() if k not in deprecated_params}
    ...
    # Apply command line overrides (filter out None values and deprecated parameters)
    deprecated_params = {'skip_time_ms'}
    overrides = {k: v for k, v in overrides.items() if v is not None and k not in deprecated_params}
...
def get_config_by_version(version, classifier_type='cwt_image'):
    """
    Retrieve configuration for a specific experiment version.

    Args:
        version: Version string (e.g., 'v042', '42', 42) or config number
        classifier_type: Type of classifier ('cwt_image' or 'pd_signal')

    Returns:
        dict: Configuration dictionary loaded from experiment log
    ...
    try:
        ...
        raise ValueError(f"Could not read experiment log: {e}")
    ...
    if len(matching_rows) == 0:
        raise FileNotFoundError(
            f"Version {version_str} not found in experiment log.\n"
            f"Available versions: {sorted(df['version'].dropna().unique())[:10]}..."
        )

    if len(matching_rows) > 1:
        print(f"Warning: Multiple entries found for {version_str}, using the first one")

    # Get the row
    row = matching_rows.iloc[0]

    # Extract config - check for config_json column first
    if 'config_json' in row and pd.notna(row['config_json']):
        try:
            config = json.loads(row['config_json'])
            return config
        except json.JSONDecodeError as e:
            raise ValueError(f"Invalid JSON in config for {version_str}: {e}")
    else:
        # Build config from individual columns
        config = {}
        for col in df.columns:
            if col not in ['version', 'timestamp', 'source', 'config_json'] and pd.notna(row[col]):
                config[col] = row[col]

        if not config:
            raise ValueError(f"No configuration data found for {version_str}")
...
{chr(10).join([f"Fold {i+1}: Accuracy={acc:.4f}, Loss={loss:.4f}" for i, (acc, loss) in enumerate(zip(experiment_results['fold_accuracies'], experiment_results['fold_losses']))])}"""
    ...
    if classifier_type == 'cwt_image':
        # Handle both old and new augmentation systems
        aug_display = config.get('augment_fraction', config.get('augment_probability', 0.0))

        config_section += f"""
- Dense Dropout: {config['dense_dropout']}
- Use Batch Norm: {config.get('use_batch_norm', False)}
- Augmentation Fraction: {aug_display}"""
...
# -------------------------
# Dataset Variant Support
# -------------------------

def load_dataset_variant_info(dataset_name):
    """
    Load dataset variant configuration and determine training mode.
    ...
    Returns:
        dict: Dataset configuration with keys:
    ...
    try:
        ...
        # Navigate up from deep CWT directory to find root
        current = cwt_data_path
        for _ in range(10):
            if 'dataset_definitions' in [d.name for d in current.iterdir() if d.is_dir()]:
                dataset_defs = current / 'dataset_definitions' / dataset_name
        ...
        # Fallback to CWT_labelled_windows level
        if not search_paths:
            current = cwt_data_path
    ...
    for path in search_paths:
        if path.exists() and (path / 'dataset_config.json').exists():
            dataset_dir = path
            break
    ...
    if dataset_dir is None:
        ...
        raise FileNotFoundError(
            f"Dataset variant '{dataset_name}' not found. Searched:\n  {searched}\n"
            f"Run: python ml/prepare_training_dataset.py --list-datasets"
        )
