...
save_cam = False # bool
show_confusion = False # bool

# Set image path and size

image_directory = r'E:/AlSi10Mg single layer ffc/CWT_ML_training_data/CWT_labelled_cw_powder_in-situ_ss_double_sliced/'
SIZE_X = 50 
SIZE_Y = 256
...
def create_dataset():
    ...
    NP_images = sorted(glob.glob(image_directory + '0/*.png'))
    for i, image_path in enumerate(NP_images):
        image = get_img_array(image_path, img_size)
        dataset.append(image)
        label.append(0)

    P_images = sorted(glob.glob(image_directory + '1/*.png'))
...
def train(X_train, X_test, y_train, y_test):
    ...
    csv_logger = CSVLogger(f'{image_directory}/CWT_image_binary_classification_training_log.csv', append=True)
    ...
    # Get latest model version in the folder and increment version number by 1
    v_num = get_latest_version(image_directory)
    model.save(f'ml/CWT_image_binary_classification_{int(v_num)+1:03}.h5')
...
def model_eval(model, X_test, y_test):
    ...
    for i, th in enumerate(thresholds, start=1):
        ...
        print(f'Threshold: {th}, Accuracy: {round(accuracy, 2)}%, Recall: {round(recall, 2)}, Precision: {round(precision, 2)}')
        ...
        plt.title(f'Confusion Matrix for CNN Classification\nThreshold={th}')
        plt.tight_layout()
        if show_confusion:
            plt.show()
        plt.close()
    ...
    for label in ('0', '1'):
        ...
        if save_cam:
            output_folder = image_directory + f'{label}/' + 'grad-CAM/'
            if not os.path.exists(output_folder):
                os.makedirs(output_folder)
        else:
            output_folder = ''
        images = os.listdir(image_directory + f'{label}/')
        cams = np.zeros((len(images), img_size[1], img_size[0]))
        for i, image_name in enumerate(images):
            if (image_name[-4:] == '.png'):
                img_path = image_directory + f'{label}/' + image_name
                ...
                cam = save_and_display_gradcam(img_path, heatmap, display=False,
                    output_path=output_folder + image_name[:-4] + '_CAM.png')
                cams[i] = cam
    ...
    ytick_indices = [0, 27, 69, 112, 154, 197, 240]
...
def get_latest_version(img_dir):
    model_list = sorted(glob.glob(img_dir + '*.h5'))
...
def main():
    ...
    v_num = get_latest_version(image_directory)
    model = load_model(Path(image_directory, f'CWT_image_binary_classification_{v_num}.h5'))
