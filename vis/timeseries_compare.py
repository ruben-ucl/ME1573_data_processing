...
running_mean_window = 9 # None or odd-valued int
...
def main():
    files = sorted(glob.glob(str(Path(path1, '1*v2.csv'))))
    all_peaks = []
    for f1 in files:
        f1name = Path(f1).name
        print('\nReading %s' % f1name)
        trackid = f1name[:7]
        f2 = glob.glob(str(Path(path2, f'{trackid}*raw.csv')))[0]
        ...
        ax2 = ax1.twinx()
        # ax2.plot(t2, var2, '--', c= 'tab:red', lw=0.8, marker='x', markersize=3, label=labels[label2][0])
