#!/usr/bin/env python3
"""
Wavelet Comparison Script

Generates test figures comparing different wavelets for CWT analysis.
Shows both the wavelet functions and their CWT results on photodiode data.

Usage:
    python vis/wavelet_comparison.py
...
def plot_wavelet_function(wavelet_name, ax):
    ...
    try:
        # Handle parameterized wavelets
        if wavelet_name in ['fbsp1-1.5-1.0', 'shan1.5-1.0', 'cgau8']:
...
def create_wavelet_comparison_figure():
    ...
    # Collect all wavelets to test
    all_wavelets = []
    for category, wavelets in WAVELETS_TO_TEST.items():
    ...
    fig.suptitle(f'Wavelet Comparison - {TARGET_TRACKID} ({series_key})', 
    ...
    for i, wavelet_name in enumerate(all_wavelets):
        ...
        category_idx = 0
        for j, (category, wavelets) in enumerate(WAVELETS_TO_TEST.items()):
            if wavelet_name in wavelets:
                category_idx = j
                break
...
def main():
    ...
    print("=" * 60)
    print("Wavelet Comparison Script")
    print("=" * 60)
    
    print(f"Target trackid: {TARGET_TRACKID}")
    print(f"Wavelets to test: {sum(len(w) for w in WAVELETS_TO_TEST.values())}")
    
    # Generate the comparison figure
    try:
        fig = create_wavelet_comparison_figure()
        ...
        if SAVE_OUTPUT:
            ...
            output_path = output_dir / f'wavelet_comparison_{TARGET_TRACKID}.png'
