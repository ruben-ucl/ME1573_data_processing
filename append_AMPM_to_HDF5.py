...
# File name to load
# trackid = input('Enter the number of the track to be analysed (format: 01SS_0T)\n')
for material_folder in glob.glob('%s/*/' % datFolder):
    for file in glob.glob('%s/*.hdf5' % material_folder):
        print('Working on %s' % str(file))
        trackid = Path(file).name
        datFile = glob.glob('%s/%s_AMPM_%s_L4_*.dat' % (AMPM_path, trackid[:4], trackid[-1]))[0]
